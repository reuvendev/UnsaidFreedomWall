...
export default function HomePage() {
  ...
  const loadMorePosts = async () => {
    if (!lastVisible || loadingMore || !hasMore || debouncedSearch !== '') {
      return;
    }
  ...
  const handleReportSubmit = async (e: React.FormEvent) => {
    ...
    if (!activeReportPostId || isSubmittingReport) {
      return;
    }
    ...
    <div className={`min-h-screen font-sans selection:bg-neutral-900 selection:text-white relative ${isDarkMode ? 'bg-neutral-950 text-neutral-100' : 'bg-neutral-50/50 text-neutral-900'}`}>
      ...
      <header className={`sticky top-0 z-50 backdrop-blur-md border-b shadow-2xs ${isDarkMode ? 'bg-neutral-900/95 border-neutral-800' : 'bg-white/95 border-neutral-200/80'}`}>
        <div className="max-w-2xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-4 sm:gap-5 font-mono text-[11px] font-bold tracking-widest uppercase">
            ...
            <Link href="/about" className={isDarkMode ? 'text-neutral-400 hover:text-white' : 'text-neutral-500 hover:text-neutral-900'}>
              About
            </Link>

            <Link href="/guidelines" className={isDarkMode ? 'text-neutral-400 hover:text-white' : 'text-neutral-500 hover:text-neutral-900'}>
              Guidelines
            </Link>

            <button onClick={toggleDarkMode} aria-label="Toggle Dark Mode" className={`p-2 rounded-xl border cursor-pointer ${isDarkMode ? 'bg-neutral-800 border-neutral-700 text-amber-400 hover:bg-neutral-700' : 'bg-neutral-100 border-neutral-200 text-neutral-700 hover:bg-neutral-200'}`}>
      ...
      <main className="max-w-2xl mx-auto px-6 pt-16 pb-24">
        ...
        <div className="relative mb-6">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search entries, keywords, or campus alias..." className={`w-full pl-10 pr-4 py-3 border rounded-xl text-sm font-mono focus:outline-none shadow-2xs ${isDarkMode ? 'bg-neutral-900 border-neutral-800 text-white placeholder:text-neutral-500 focus:border-emerald-500' : 'bg-white border-neutral-200 text-neutral-900 placeholder:text-neutral-400 focus:border-neutral-900 focus:ring-1 focus:ring-neutral-900'}`} />
        ...
        <div className={`flex items-center gap-2 overflow-x-auto pb-4 mb-8 border-b hide-scrollbar ${isDarkMode ? 'border-neutral-800' : 'border-neutral-200/80'}`}>
          ...
          {CATEGORIES.map((cat) => (
            <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} className={`px-4 py-2 text-xs font-mono font-semibold uppercase tracking-wider rounded-lg whitespace-nowrap ${selectedCategory === cat.id ? 'bg-neutral-900 dark:bg-emerald-600 text-white shadow-sm' : isDarkMode ? 'bg-neutral-900 text-neutral-400 border border-neutral-800 hover:bg-neutral-800 hover:text-white' : 'bg-white text-neutral-600 border border-neutral-200/80 hover:bg-neutral-100 hover:text-neutral-900'}`}>
              {cat.label}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            {posts.map((post) => {
              ...
              return (
                <article key={post.id} className={`p-5 sm:p-6 rounded-2xl relative group hover:-translate-y-1 hover:shadow-xl ${isDev ? isDarkMode ? 'bg-emerald-950/20 border-2 border-emerald-500/50 shadow-md ring-1 ring-emerald-500/10' : 'bg-emerald-50/50 border-2 border-emerald-500/60 shadow-md ring-1 ring-emerald-500/20' : isDarkMode ? 'bg-neutral-900 border border-neutral-800 shadow-xs hover:border-neutral-700' : 'bg-white border border-neutral-200/80 shadow-xs hover:border-neutral-300'}`}>
                  ...
                  <div className={`flex flex-wrap items-center justify-between gap-y-2 mb-3 ${isDev ? 'mt-1' : ''}`}>
                    ...
                    <div className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-wider min-w-0">
                      ...
                      <span className={`px-2.5 py-1 rounded-md border font-bold truncate max-w-[150px] sm:max-w-none ${isDev ? isDarkMode ? 'bg-emerald-950 text-emerald-300 border-emerald-800' : 'bg-emerald-100/80 text-emerald-900 border-emerald-200' : isDarkMode ? 'bg-neutral-800 text-neutral-300 border-neutral-700' : 'bg-neutral-100 text-neutral-800 border-neutral-200/60'}`}>
                        {post.authorAlias}
                      </span>
                    ...
                    <span className={`text-[10px] font-mono uppercase tracking-widest px-2.5 py-1 rounded-md shrink-0 ${isDev ? 'bg-emerald-100 text-emerald-800 font-bold' : isDarkMode ? 'bg-neutral-800 text-neutral-400 border border-neutral-700/50' : 'bg-neutral-100/80 text-neutral-600 border border-neutral-200/40'}`}>
                      {post.category}
                    </span>
                  ...
                  <div className="mb-6">
                    ...
                    <p className={`text-base sm:text-lg md:text-xl font-normal leading-relaxed break-words whitespace-pre-wrap ${isDev ? isDarkMode ? 'text-emerald-200 font-semibold' : 'text-emerald-950 font-semibold' : isDarkMode ? 'text-neutral-200' : 'text-neutral-800'}`}>
                      {displayContent}
                    </p>

                    {isLongContent && (
                      <button onClick={() => toggleExpand(post.id)} className="mt-2 text-xs font-mono font-bold uppercase tracking-wider text-emerald-500 hover:text-emerald-400 inline-block focus:outline-none">
                  ...
                  {/* IMAGE ATTACHMENT */}
                  {post.imageUrl && <div className="mb-6 overflow-hidden rounded-xl border border-neutral-200/80 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-950"><img src={post.imageUrl} alt="Attached image" loading="lazy" className="w-full max-h-[600px] object-contain" /></div>}

                  {/* SPOTIFY */}
                  {post.spotifyTrackId && <div className="mb-6"><iframe src={`https://open.spotify.com/embed/track/${post.spotifyTrackId}?utm_source=generator&theme=${isDarkMode ? '1' : '0'}`} width="100%" height="80" frameBorder="0" allow="encrypted-media" className={`rounded-xl border shadow-2xs ${isDarkMode ? 'border-neutral-800' : 'border-neutral-100'}`} /></div>}

                  {/* ACTIONS */}
                  <div className={`flex flex-wrap items-center justify-between gap-y-3 pt-4 border-t ${isDev ? isDarkMode ? 'border-emerald-900/40' : 'border-emerald-200/60' : isDarkMode ? 'border-neutral-800' : 'border-neutral-100'}`}>

                    <div className="flex items-center gap-4 sm:gap-6 font-mono text-xs font-semibold">

                      {/* LIKE */}
                      <button onClick={() => handleVoteToggle(post.id)} disabled={isLocked} className={`flex items-center gap-2 ${isLocked ? 'opacity-50 cursor-not-allowed' : ''} ${hasVoted ? 'text-rose-500 hover:text-rose-600' : isDev ? 'text-emerald-600 hover:text-rose-500' : isDarkMode ? 'text-neutral-400 hover:text-rose-500' : 'text-neutral-500 hover:text-rose-500'}`}>
                        <Icons.Heart filled={hasVoted} />
                        <span>{post.upvotes}</span>
                      </button>

                      {/* REPLIES */}
                      <Link href={`/post/${post.id}`} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 cursor-pointer ${isDev ? 'text-emerald-600 hover:text-emerald-400' : isDarkMode ? 'text-neutral-400 hover:text-white' : 'text-neutral-500 hover:text-neutral-900'}`}>
                        <Icons.Message />
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {/* REPORT */}
                      {!isDev && (
                        <button onClick={() => setActiveReportPostId(post.id)} disabled={isReported} className="font-mono text-[11px] text-neutral-400 hover:text-rose-600 uppercase tracking-wider disabled:opacity-50">
                          {isReported ? 'Reported' : 'Report'}
                        </button>
                      )}

                      {/* SHARE */}
                      <button onClick={() => handleShare(post.id)} className={`flex items-center gap-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider ${isDev ? 'text-emerald-600 hover:text-emerald-400' : isDarkMode ? 'text-neutral-400 hover:text-white' : 'text-neutral-400 hover:text-neutral-900'}`}>
                        <Icons.Share />
            ...
            {hasMore && searchQuery.trim() === '' && (
              <div className="pt-6 text-center">
                ...
                <button onClick={loadMorePosts} disabled={loadingMore} className={`px-6 py-3 border font-mono text-xs font-bold uppercase tracking-wider rounded-xl disabled:opacity-50 shadow-2xs ${isDarkMode ? 'bg-neutral-900 border-neutral-800 text-neutral-300 hover:bg-neutral-800' : 'bg-white border-neutral-200 text-neutral-700 hover:bg-neutral-100'}`}>
      ...
      {activeReportPostId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-950/60 backdrop-blur-xs">
          ...
          <div className={`w-full max-w-md rounded-2xl shadow-xl border overflow-hidden ${isDarkMode ? 'bg-neutral-900 border-neutral-800 text-white' : 'bg-white border-neutral-200/80 text-neutral-900'}`}>
            ...
            <div className={`flex items-center justify-between px-6 py-4 border-b ${isDarkMode ? 'border-neutral-800' : 'border-neutral-100'}`}>
              ...
              <button onClick={() => setActiveReportPostId(null)} className="text-neutral-400 hover:text-white p-1 rounded-lg">
                <Icons.Close />
              </button>
            ...
            <form onSubmit={handleReportSubmit} className="p-6 space-y-5">
              ...
              <div>
                ...
                <select value={selectedReason} onChange={(e) => setSelectedReason(e.target.value)} className={`w-full p-3 border rounded-xl text-xs font-mono focus:outline-none appearance-none cursor-pointer ${isDarkMode ? 'bg-neutral-950 border-neutral-800 text-white focus:border-emerald-500' : 'bg-neutral-50 border-neutral-200 text-neutral-900 focus:border-neutral-900'}`}>
              ...
              <div>
                ...
                <textarea value={reportDetails} onChange={(e) => setReportDetails(e.target.value)} placeholder="Provide any extra context for moderators..." rows={3} className={`w-full p-3 border rounded-xl text-xs placeholder:text-neutral-500 font-mono focus:outline-none resize-none shadow-2xs ${isDarkMode ? 'bg-neutral-950 border-neutral-800 text-white focus:border-emerald-500' : 'bg-neutral-50 border-neutral-200 text-neutral-900 focus:border-neutral-900'}`} />
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button type="button" onClick={() => setActiveReportPostId(null)} className={`px-4 py-2.5 border rounded-xl font-mono text-xs uppercase font-bold tracking-wider ${isDarkMode ? 'bg-neutral-800 border-neutral-700 text-neutral-300 hover:bg-neutral-700' : 'bg-white border-neutral-200 text-neutral-700 hover:bg-neutral-100'}`}>
                  Cancel
                </button>

                <button type="submit" disabled={isSubmittingReport} className="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-mono text-xs uppercase font-bold tracking-wider disabled:opacity-50 shadow-sm">
