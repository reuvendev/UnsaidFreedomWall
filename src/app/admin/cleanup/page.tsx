...
const Icons = {
  ...
  Trash: () => (
    ...
    >
      <path d="M3 6h18" />
      <path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-2-2-2V6" />
...
export default function AdminCleanupPage() {
  ...
  const fetchRoomMetrics = async () => {
    ...
    try {
      ...
      roomsSnap.docs.forEach((docSnap) => {
        ...
        if (
          data.createdAt &&
          data.createdAt < twentyFourHoursAgo
        ) {
          expired++;
        }
      });
      ...
      try {
        const counterDoc = await getDoc(
          doc(db, 'counters', 'system')
        );

        if (
          counterDoc.exists() &&
          typeof counterDoc.data().totalCreated === 'number'
        ) {
          allTime =
            counterDoc.data().totalCreated;
        }
      } catch (e) {
        console.warn(
          'Could not fetch remote counter, using fallback/estimate'
        );
      }
      ...
      addLog(
        'Successfully synchronized database metrics.',
        'info'
      );
    } catch (err) {
      console.error(
        'Error fetching room metrics:',
        err
      );

      addLog(
        'Failed to synchronize database metrics.',
        'error'
      );
    } finally {
      setFetching(false);
  ...
  const handleAction = async (
    ...
    const confirmationText =
      actionType === 'purge_stale'
        ? 'Purge all stuck waiting rooms older than 2 minutes, including their messages?'
        : actionType === 'delete_expired'
          ? 'Permanently delete all chat rooms older than 24 hours, INCLUDING ALL THEIR MESSAGES?'
          : 'Execute full database housekeeping sweep, including messages?';
    ...
    addLog(
      `Initiated protocol: ${actionType.replace(
        '_',
        ' '
      )}...`,
      'info'
    ...
    try {
      ...
      /*
       * Firestore batches can contain a maximum of
       * 500 write operations.
       *
       * We use 450 to leave a safety margin.
       */
      ...
      for (const roomDoc of snapshot.docs) {
        ...
        /*
         * Determine whether this room should be deleted.
         */
        ...
        const shouldDeleteStale =
          (actionType ===
            ...
            actionType ===
              'full_sweep') &&
          data.status === 'waiting' &&
          createdAt < staleWaitingLimit;

        const shouldDeleteExpired =
          (actionType ===
            'delete_expired' ||
            actionType ===
              'full_sweep') &&
         ...
         * --------------------------------------------------------
         * STEP 1:
         * Delete all messages inside this room.
         * --------------------------------------------------------
         */
      ...
      /*
       * Commit anything left in the final batch.
       */
      ...
      if (
        ...
        addLog(
          `Deleted ${roomsDeleted.toLocaleString()} chat room(s) and ${messagesDeleted.toLocaleString()} message(s).`,
          'success'
        );
      } else {
        addLog(
          'Target collections are already clean. No modifications needed.',
          'info'
        );
      }
      ...
      addLog(
        'Operation execution failed. Review browser console.',
        'error'
      );
    } finally {
      setLoading(null);
   ...
   * ============================================================
   *
   * This specifically handles messages left behind by your OLD
   * cleanup code.
   *
   * Example:
   *
   ...
   *       ❌ deleted
   *
   * /chatRooms/ABC123/messages/message1
   *       ⚠️ still exists
   *
   * collectionGroup("messages") allows us to find those messages
   * even though the parent room document no longer exists.
   */
  const handleCleanOrphanedMessages =
    async () => {
      const confirmed = confirm(
        'This will permanently delete ALL message documents whose parent chat room no longer exists. This action cannot be undone. Continue?'
      );
      ...
      setLoading('clean_orphans');

      addLog(
        'Initiated orphaned message cleanup...',
        'info'
      );

      try {
        /*
         * Find EVERY "messages" subcollection in Firestore.
         *
         * This includes messages under rooms whose parent
         * chatRooms document has already been deleted.
         */
        const messagesSnapshot =
          await getDocs(
            collectionGroup(
        ...
        addLog(
          `Scanned ${messagesSnapshot.size.toLocaleString()} message document(s).`,
          'info'
        );
        ...
        if (
          ...
          addLog(
            'No message documents found. Database is clean.',
            'success'
          );
        ...
        /*
         * Build a cache of parent room existence.
         *
         * Without caching, if a room has 50 messages,
         * we'd perform 50 identical parent lookups.
         */
        const parentRoomCache =
          new Map<
            string,
            boolean
          >();
        ...
        for (
          ...
          /*
           * This should always exist for your structure,
           * but protect against unexpected paths.
           */
          if (!roomRef) {
            continue;
          }
          ...
          /*
           * Only check Firestore once per room.
           */
          if (
           ...
           *
           * Therefore this message is orphaned.
           */
          if (!roomExists) {
            batch.delete(
              messageDoc.ref
        ...
        if (
          ...
          addLog(
            `Removed ${orphanedMessages.toLocaleString()} orphaned message(s) from ${checkedRooms.toLocaleString()} missing room(s).`,
            'success'
          );
        } else {
          addLog(
            'No orphaned messages were found. Database is already clean.',
            'success'
          );
        }
        ...
        addLog(
          'Orphan cleanup failed. Check Firestore security rules and browser console.',
          'error'
        );
      } finally {
        setLoading(null);
        ...
        <div className="w-full max-w-md p-8 bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs font-mono text-neutral-400">
              Environment-secured authentication required.
            </p>
          </div>
      ...
      <header className="sticky top-0 z-50 border-b border-neutral-800 bg-neutral-900/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link
              href="/"
              className="p-2 bg-neutral-800/60 hover:bg-neutral-800 border border-neutral-700/50 rounded-xl transition text-neutral-300 hover:text-white"
              title="Return Home"
            >
              <Icons.ArrowLeft />
            </Link>
            ...
            <div className="font-mono text-lg font-black tracking-tighter">
              ...
              <span className="text-xs font-normal text-neutral-400 font-sans uppercase tracking-widest ml-2">
                Admin Console
              </span>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 w-full flex-1 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-neutral-800/80 pb-6">
          <div>
            ...
            <p className="text-sm text-neutral-400 font-mono mt-1">
              Monitor room usage, execute maintenance sweeps, and purge memory leaks.
            </p>
          </div>
          ...
          <button
            ...
            disabled={fetching || loading !== null}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
          <div className="bg-neutral-900/60 border border-neutral-800/80 p-6 rounded-2xl backdrop-blur-sm flex flex-col justify-between">
            ...
            <div className="text-4xl font-black text-white mt-4 font-mono">
              {formatNumber(
                stats.allTimeCreated
              )}
            </div>

            <span className="text-[11px] text-neutral-500 font-mono mt-2">
              Cumulative historical rooms
            </span>
          </div>
          ...
          <div className="bg-neutral-900/60 border border-neutral-800/80 p-6 rounded-2xl backdrop-blur-sm flex flex-col justify-between">
            ...
            <div className="text-4xl font-black text-neutral-200 mt-4 font-mono">
              {formatNumber(
                stats.totalRooms
              )}
            </div>
          ...
          <div className="bg-neutral-900/60 border border-neutral-800/80 p-6 rounded-2xl backdrop-blur-sm flex flex-col justify-between">
            ...
            <div className="text-4xl font-black text-blue-400 mt-4 font-mono">
              {formatNumber(
                stats.activeRooms
              )}
            </div>
          ...
          <div className="bg-neutral-900/60 border border-neutral-800/80 p-6 rounded-2xl backdrop-blur-sm flex flex-col justify-between">
            ...
            <div className="text-4xl font-black text-amber-400 mt-4 font-mono">
              {formatNumber(
                stats.waitingRooms
              )}
            </div>
          ...
          <div className="bg-neutral-900/60 border border-neutral-800/80 p-6 rounded-2xl backdrop-blur-sm flex flex-col justify-between">
            ...
            <div className="text-4xl font-black text-rose-400 mt-4 font-mono">
              {formatNumber(
                stats.expiredRooms
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-1 bg-neutral-900/80 border border-neutral-800 rounded-2xl p-6 space-y-6">
            <div className="flex items-center gap-3 border-b border-neutral-800 pb-4">
              ...
              <div>
                ...
                <p className="text-[11px] font-mono text-neutral-400">
                  Trigger manual database scripts
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={
                  handleCleanOrphanedMessages
                }
                disabled={
                  loading !== null
                }
                className="w-full py-3.5 px-4 bg-rose-950/30 hover:bg-rose-950/50 border border-rose-900/50 active:scale-[0.98] text-rose-300 font-mono text-xs font-bold uppercase tracking-wider rounded-xl transition flex items-center justify-between cursor-pointer disabled:opacity-50"
              >
                <span>
                  Clean Orphaned Messages
                </span>
            ...
            <div className="flex items-center justify-between border-b border-neutral-800 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] font-mono text-neutral-400">
                    Live operational event tracking
                  </p>
                </div>
              </div>
