...
import { collection, getDocs, query, orderBy, doc, deleteDoc, getDoc, updateDoc, increment } from 'firebase/firestore';
...
export default function AdminReportsPage() {
      ...
      for (const docSnap of querySnapshot.docs) {
        ...
        if (contentType === 'reply' && postId && replyId) {
          ...
          if (replySnap.exists()) {
            ...
            contentSnippet = replyData.content || "";
            authorAlias = replyData.authorAlias || "UNSAID #00000";
          }
        } else if (postId) {
        ...
        fetchedReports.push({
          ...
          details: data.details || data.contentSnippet || "",
  ...
  const handleDeleteContent = async (report: ReportItem) => {
    const targetType = report.contentType === 'reply' ? 'reply' : 'post';
    const confirmed = window.confirm(`Are you sure you want to permanently delete this reported ${targetType}?`);
    if (!confirmed) return;

    try {
      if (report.contentType === 'reply' && report.postId && report.replyId) {
        // 1. Delete the reply document
        await deleteDoc(doc(db, "posts", report.postId, "replies", report.replyId));

        // 2. Decrement the parent post's reply counter safely
              ...
              <div key={report.id} className="p-6 bg-neutral-950 border border-neutral-800 rounded-lg space-y-4">
                <div className="flex flex-wrap items-center justify-between text-xs font-mono gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="bg-neutral-800 text-neutral-300 px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider">
                      {report.contentType === 'reply' ? 'Reply' : 'Post'}
                    </span>
                ...
                <div className="space-y-1">
                  <span className="font-mono text-[10px] text-neutral-500 uppercase tracking-widest">
                    Reported {report.contentType === 'reply' ? 'Reply' : 'Post'} Content:
                  </span>
                ...
                {report.details && report.details !== report.contentSnippet && (
                  <div className="space-y-1">
                    <span className="font-mono text-[10px] text-neutral-500 uppercase tracking-widest">Reporter's Notes / Snippet:</span>
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      Delete {report.contentType === 'reply' ? 'Reply' : 'Post'}
                    </button>
                  </div>
                </div>
              </div>
